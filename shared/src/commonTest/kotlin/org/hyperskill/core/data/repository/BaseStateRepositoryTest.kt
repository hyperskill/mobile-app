...
class BaseStateRepositoryTest {
    ...
    @Test
    fun `Single request should be executed`() = runTest {
    ...
    @Test
    fun `Concurrent forceUpdate call should trigger only one state loading`() = runTest {
        var loadStateCallsCount = 0
        val repository = TestStateRepository(
            externalLoadState = {
                loadStateCallsCount++
    ...
    @Test
    fun `Concurrent cached and forceUpdate call should returns expected results`() = runBlocking {
