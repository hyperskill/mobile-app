...
class TopicsToDiscoverNextReducer : StateReducer<State, Message, Action> {
        ...
        when (message) {
            is Message.Initialize ->
                if (state is State.Idle ||
                    ...
                    State.Loading to setOf(Action.FetchTopicsToDiscoverNext(message.screen.sentryTransaction))
                } else {
                    null
                }
                ...
                when (state) {
                    is State.Content ->
                        if (state.isRefreshing) null
                            ...
                            Action.FetchTopicsToDiscoverNext(message.screen.sentryTransaction)
                        )
                    ...
                    state to setOf(
                        ...
                        Action.LogAnalyticEvent(
                            TopicsToDiscoverNextClickedHyperskillAnalyticEvent(
                                analyticRoute = message.screen.analyticRoute,
