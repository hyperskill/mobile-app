...
enum HTMLString {
    ...
    private static let htmlEscapeRegex = try! NSRegularExpression(
        pattern: "&#?[a-zA-Z0-9]+?;",
        options: .caseInsensitive
    )

    /// Replaces every HTML entity in the receiver with the matching Unicode character.
    static func unescape(string: String) -> String {
        let matches = htmlEscapeRegex.matches(
            in: string,
        ...
        for match in matches {
            let nsString = resultString as NSString

            let range = NSRange(location: match.range.location - offset, length: match.range.length)
            let entity = nsString.substring(with: range)

            if let decodedEntity = decode(entity) {
                resultString = nsString.replacingCharacters(in: range, with: String(decodedEntity))
    ...
    private static func decode(_ entity: String) -> Character? {
        if entity.hasPrefix("&#x") || entity.hasPrefix("&#X") {
            ...
            return kCharacterEntities[entity]
        }
    }
...
private let kCharacterEntities: [String: Character] = [
