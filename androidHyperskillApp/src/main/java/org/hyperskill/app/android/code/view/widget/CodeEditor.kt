...
constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    private val prettifyParser: PrettifyParser
    private val codeAnalyzer: CodeAnalyzer

    init {
        val platformCodeEditorComponent = HyperskillApp.graph().buildPlatformCodeEditorComponent()
        prettifyParser = platformCodeEditorComponent.prettifyParser
        codeAnalyzer = platformCodeEditorComponent.codeAnalyzer
    }
    ...
    private val onGlobalLayoutListener =
        ViewTreeObserver.OnGlobalLayoutListener { layoutChangesPublisher.onNext(RxEmpty.INSTANCE) }
    private val onScrollChangedListener =
        ViewTreeObserver.OnScrollChangedListener { layoutChangesPublisher.onNext(RxEmpty.INSTANCE) }
    ...
    override fun onDraw(canvas: Canvas) {
        val lineNumbersOffset = lineNumbersTextPaint.measureText(lineCount.toString())
            .toInt() + 2 * LINE_NUMBERS_MARGIN_PX

        if (paddingLeft != lineNumbersOffset + LINE_NUMBERS_MARGIN_PX) {
            setPadding(
                lineNumbersOffset + LINE_NUMBERS_MARGIN_PX,
                paddingTop,
                paddingRight,
                paddingBottom
            )
        ...
        canvas.drawRect(
            0f,
            0f,
            lineNumbersOffset.toFloat(),
            height.toFloat(),
            lineNumbersBackgroundPaint
        ) // line numbers bg
        canvas.drawLine(
            lineNumbersOffset.toFloat(),
            0f,
        ...
        if (layout != null) {
            ...
            linesWithNumbers.forEachIndexed { lineNumber, line ->
                ...
                canvas.drawText(
                    ...
                    lineNumbersOffset.toFloat() - LINE_NUMBERS_MARGIN_PX,
                    y.toFloat(),
                    lineNumbersTextPaint
                )
            }
    ...
    private fun highlightBracket(firstBracketPos: Int, bracket: String) {
        val secondBracketPos =
            codeAnalyzer.findSecondBracket(bracket, firstBracketPos, editableText.toString())
        if (secondBracketPos != -1) {
            editableText.setSpan(
                CodeHighlightSpan(theme.bracketsHighlight),
                firstBracketPos,
                firstBracketPos + 1,
            ...
            editableText.setSpan(
                CodeHighlightSpan(theme.errorHighlight),
                firstBracketPos,
                firstBracketPos + 1,
                Spannable.SPAN_EXCLUSIVE_EXCLUSIVE
            )
    ...
    private fun getFirstVisibleLine() =
        scrollContainer?.let {
            return@let layout.getLineForVertical(max(0, it.scrollY - top))
        } ?: 0

    private fun getLastVisibleLine() =
        scrollContainer?.let {
